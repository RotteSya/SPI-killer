...
最新状态（2026-09-10）：发布分支代码 `402d265` 已修复远端配置的账号/服务绑定、迟到响应、运行时缓存过期以及响应限量/重定向保护；新增 5 项回归，完整 Swift 302 项、2 项模型评测跳过、0 失败，CI 10/10。新 DMG 已重新公证、装订及 Gatekeeper 通过，62 个打包输入与代码一致。真实新合约答案/解释及断线后免费恢复请求沿用已记录证据，恢复/解释完成界面和整体稳定性仍待验收；生产尚未发布。以 [9 月 10 日发布记录](docs/release-progress-2026-09-10.md) 为当前结果，后面的早期记录保留历史边界。
