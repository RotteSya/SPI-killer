...
这些是待审材料，尚未形成正式 400 题 holdout：多选/排序、异常与布局覆盖、家族隔离、单位/语义真值及独立复核仍待完成；公开基准可能被模型预训练见过，不能声称新鲜或替代全部真实场景样本。ScholarBench 下载未匹配上游固定版本声明的字节数，已拒绝纳入题集并保留失败记录。来源许可、候选取舍与具体证据见 [公开题集获取记录](public-corpus-acquisition-2026-09-09.md)。没有因此放宽质量闸门、启用收费测试或更改生产。
